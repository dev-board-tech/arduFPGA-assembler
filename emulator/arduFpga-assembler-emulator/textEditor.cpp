...
static void paintWindow(textEditor_t *inst) {
    ...
    if((inst->gfxString.edgeTouch & inst->gfxString.EDGE_OUTSIDE_LEFT) && inst->winX + getCharWidth4x6() <= 0) {
        inst->winX += getCharWidth4x6();
        inst->edited = true;
    }
    if(inst->gfxString.edgeTouch & inst->gfxString.EDGE_OUTSIDE_RIGHT) {
        inst->winX -= getCharWidth4x6();
        inst->edited = true;
    }
    if(inst->gfxString.edgeTouch & inst->gfxString.EDGE_OUTSIDE_UP || inst->cursorPos <= inst->winY) {
        inst->winY = getPreviousLine(inst, inst->winY);
        inst->lineCnt--;
        inst->edited = true;
    }
    if(inst->gfxString.edgeTouch & inst->gfxString.EDGE_OUTSIDE_DOWN) {
...
textEditor_Err_e addFile(textEditor_t *inst, char *fileName) {
    ...
    int fileCnt = 0;
    for(; ptr[fileCnt] != nullptr; fileCnt++) {}
...
static void moveString(textEditor_t *inst, textEditorPtr from, textEditorPtr to) {
    // Move string left or right in chuncks of sizeof(chunk) buffer, in case the text buffer is in an external serial memory.
    char chunk[16];
    if(from < to) {
...
static int _drawStringWindowed4x6(textEditor_t *inst, box_t *box, textEditorPtr string, int16_t x, int16_t y, int16_t cursorPos, bool cursorState) {
    ...
    do {
        ...
        if(cursorPos == cCnt) {
            if(cX < box__.x_min)
                inst->gfxString.edgeTouch |= inst->gfxString.EDGE_OUTSIDE_LEFT;
            if(cY < box__.y_min)
                inst->gfxString.edgeTouch |= inst->gfxString.EDGE_OUTSIDE_UP;
            if(cX >= box__.x_max)
                inst->gfxString.edgeTouch |= inst->gfxString.EDGE_OUTSIDE_RIGHT;
            if(cY >= box__.y_max - 6)
                inst->gfxString.edgeTouch |= inst->gfxString.EDGE_OUTSIDE_DOWN;
        }
