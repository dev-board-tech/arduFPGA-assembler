...
int ff_util_appendhexs (
    ...
    const int buff_len,/* Number of bytes to be output */
	FIL* fp				/* Pointer to the file object */
)
{
    int cnt = 0;
...
#endif
{
	...
	VA_SHIFT (fmt, char *);
    char buff[1];
    long string_len = 0;//ff_vsnprintf_(fp, buff, 65535, str, ap);
	VA_END;
	if(f_sync(fp) != FR_OK)
		return 0;
